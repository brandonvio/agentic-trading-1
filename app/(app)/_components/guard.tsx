 ...
 * lacks the permission; a quiet failure block for anything else. Mirrors the
 * dashboard's guard so every section degrades the same way.
 */
...
/** Page-level variant: full-height block for when the whole route is denied. */
export function PageGuard<T>({ result, what, children }: { result: SafeResult<T>; what: string; children: (value: T) => ReactNode }) {
  if (result.ok) return <>{children(result.value)}</>;
  return isForbidden(result) ? (
    <NotPermitted what={what} compact={false} />
  ) : (
    <EmptyState tone="negative" title="Unavailable" description={result.error.message} />
  );
}
